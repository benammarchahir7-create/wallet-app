...
const fmt = (n) => n.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+" EUR";
...
const parseDate = (str) => {
  ...
  try {
    ...
    if(pts.length>=3){ const day=parseInt(pts[0]); const yr=parseInt(pts[pts.length-1]); const mon=MONTHS.findIndex(m=>pts[1].toLowerCase().startsWith(m)); return new Date(yr,mon<0?0:mon,day); }
  } catch(e){}
  return new Date();
};
...
const CARD_PALETTES = {
  mono:    [{grad:"linear-gradient(145deg,#1C1C1E,#2C2C2E)",accent:"#C9A84C",light:false},{grad:"linear-gradient(145deg,#F2EDE4,#E2D9CC)",accent:"#8B6040",light:true},{grad:"linear-gradient(145deg,#111,#1E1E1E)",accent:"#E8E2D6",light:false},{grad:"linear-gradient(145deg,#272218,#332C1E)",accent:"#D4AF37",light:false}],
  vivid:   [{grad:"linear-gradient(145deg,#E8445A,#c0392b)",accent:"#ff8a96",light:false},{grad:"linear-gradient(145deg,#3B82F6,#1d4ed8)",accent:"#93c5fd",light:false},{grad:"linear-gradient(145deg,#F59E0B,#b45309)",accent:"#fef08a",light:false},{grad:"linear-gradient(145deg,#8B5CF6,#5b21b6)",accent:"#c4b5fd",light:false}],
  aurora:  [{grad:"linear-gradient(145deg,#0f2027,#2c5364)",accent:"#4dd9ac",light:false},{grad:"linear-gradient(145deg,#1a1a2e,#0f3460)",accent:"#e94560",light:false},{grad:"linear-gradient(145deg,#134e5e,#71b280)",accent:"#a7f3d0",light:false},{grad:"linear-gradient(145deg,#0f0c29,#302b63)",accent:"#818cf8",light:false}],
  sakura:  [{grad:"linear-gradient(145deg,#fce4ec,#f8bbd0)",accent:"#c2185b",light:true},{grad:"linear-gradient(145deg,#f3e5f5,#e1bee7)",accent:"#7b1fa2",light:true},{grad:"linear-gradient(145deg,#fff3e0,#ffe0b2)",accent:"#e65100",light:true},{grad:"linear-gradient(145deg,#e3f2fd,#bbdefb)",accent:"#1565c0",light:true}],
  neon:    [{grad:"linear-gradient(145deg,#060612,#0d0d20)",accent:"#00ff88",light:false},{grad:"linear-gradient(145deg,#000d1a,#001a33)",accent:"#00d4ff",light:false},{grad:"linear-gradient(145deg,#0d0d0d,#1a0a2e)",accent:"#bf00ff",light:false},{grad:"linear-gradient(145deg,#1a0000,#2d0000)",accent:"#ff2d55",light:false}],
  sunset:  [{grad:"linear-gradient(145deg,#7b2d00,#b34700)",accent:"#ffb347",light:false},{grad:"linear-gradient(145deg,#4a0e0e,#7a1c1c)",accent:"#ff6b6b",light:false},{grad:"linear-gradient(145deg,#2d1b00,#5c3800)",accent:"#ffd166",light:false},{grad:"linear-gradient(145deg,#0a2e2e,#0d4f4f)",accent:"#06d6a0",light:false}],
...
const getCT = (theme, idx) => (CARD_PALETTES[theme]||CARD_PALETTES.mono)[idx%4];
...
const BODY_H = 58, SHOW_PX = 34, PEEK = 96, CARD_H = 210;
...
const INIT_TICKETS = [
  ...
  {id:3,label:"SNCF",       amount:124.0, date:daysAgo(8), items:["Paris Lyon TGV 2nde cl."],cardTheme:2,archived:false},
  ...
  {id:7,label:"Leclerc",    amount:112.30,date:daysAgo(8), items:["Viande","Legumes","Fromage"],cardTheme:6,archived:true,archiveFolder:"a1"},
...
const TicketCard = ({ticket,index,total,onClick,uiTheme}) => {
  const ct = getCT(uiTheme, ticket.cardTheme||0);
  const tc = ct.light?"rgba(20,10,0,0.9)":"rgba(255,255,255,0.95)";
  const sc = ct.light?"rgba(20,10,0,0.45)":"rgba(255,255,255,0.35)";
  return (
    <div className="wcard" onClick={onClick} style={{position:"absolute",bottom:(total-1-index)*PEEK,left:0,right:0,height:CARD_H,cursor:"pointer",zIndex:index,filter:ct.light?"drop-shadow(0 -2px 1px rgba(0,0,0,0.14))":"drop-shadow(0 20px 30px rgba(0,0,0,0.6))"}}>
      <div className="rshape" style={{width:"100%",height:"100%",background:ct.grad,border:`1px solid ${ct.light?"rgba(0,0,0,0.08)":"rgba(255,255,255,0.1)"}`,padding:"18px 20px 16px",display:"flex",flexDirection:"column"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
          <div style={{width:40,height:40,borderRadius:12,background:`${ct.accent}20`,border:`1px solid ${ct.accent}30`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <Icon type="receipt" size={20} color={ct.accent}/>
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:tc,fontSize:19,fontWeight:700,fontFamily:"'Courier Prime',monospace",lineHeight:1.1}}>{ticket.label}</div>
            <div style={{color:sc,fontSize:10,fontFamily:"'Courier Prime',monospace",marginTop:2}}>{ticket.date}</div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{color:ct.accent,fontSize:20,fontWeight:700,fontFamily:"'Space Mono',monospace",letterSpacing:-1,lineHeight:1}}>{fmt(ticket.amount)}</div>
            <div style={{color:sc,fontSize:8,fontFamily:"'Space Mono',monospace",marginTop:2,textTransform:"uppercase"}}>Total</div>
          </div>
        </div>
        <div style={{borderTop:`1px dashed ${sc}`,marginBottom:8,opacity:0.4}}/>
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:4}}>
          {ticket.items?.slice(0,3).map((item,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",color:sc,fontSize:11,fontFamily:"'Courier Prime',monospace"}}>
              <span>{item}</span><span>...</span>
            </div>
          ))}
          {(ticket.items?.length||0)>3&&<div style={{color:sc,fontSize:10,fontStyle:"italic",fontFamily:"'Courier Prime',monospace"}}>+ {ticket.items.length-3} autres</div>}
        </div>
        <div style={{borderTop:`1px dashed ${sc}`,marginTop:4,paddingTop:6,opacity:0.4}}/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <Barcode color={tc} opacity={0.35}/>
          <div style={{color:sc,fontSize:7,fontFamily:"'Space Mono',monospace",letterSpacing:0.5}}>TXN-{String(ticket.id).padStart(9,"0")}</div>
        </div>
      </div>
    </div>
  );
...
const ArchiveFolder = ({folder,tickets,index,isOpen,onToggle,onEdit,onDelete,uiTheme}) => (
  <div style={{position:"relative",zIndex:isOpen?100:10+index,marginBottom:isOpen?12:-(BODY_H-SHOW_PX),transition:"margin-bottom 0.4s cubic-bezier(0.4,0,0.2,1)",transform:isOpen?"scale(1.012)":"scale(1)"}}>
    ...
    <div onClick={onToggle} style={{background:"linear-gradient(175deg,#F7E070 0%,#EEBF35 65%,#D9A820 100%)",borderRadius:14,cursor:"pointer",boxShadow:isOpen?"0 16px 36px rgba(0,0,0,0.4)":"0 4px 12px rgba(0,0,0,0.25)",border:"1px solid rgba(139,96,32,0.22)"}}>
    ...
    <div style={{maxHeight:isOpen?500:0,overflow:"hidden",transition:"max-height 0.45s cubic-bezier(0.4,0,0.2,1),opacity 0.3s",opacity:isOpen?1:0}}>
      <div style={{background:"#1a1a1a",borderRadius:"0 0 14px 14px",border:"1px solid rgba(255,255,255,0.07)",borderTop:"none",padding:"6px 0"}}>
        ...
        {tickets.map((t,i)=>{
          const ct=getCT(uiTheme,t.cardTheme||0);
          return <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:i<tickets.length-1?"1px solid rgba(255,255,255,0.04)":"none"}}>
            <div style={{width:28,height:28,borderRadius:8,background:ct.grad,flexShrink:0}}/>
            <div style={{flex:1,minWidth:0}}>
...
const BarChart = ({data,color}) => {
  ...
  return <div style={{display:"flex",alignItems:"flex-end",gap:4,height:60}}>{data.map((d,i)=>(
    <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
      <div style={{width:"100%",borderRadius:"4px 4px 0 0",background:i===data.length-1?color:"rgba(255,255,255,0.1)",height:Math.max(4,(d.v/max)*52),minHeight:4}}/>
...
export default function App() {
  ...
  const cv = (v,cb) => { setViewAnim("out"); setTimeout(()=>{ setView(v); setViewAnim("in"); cb&&cb(); },150); };
  const goSearch = () => cv("search",()=>setTimeout(()=>searchRef.current&&searchRef.current.focus(),60));
  const toast_ = (msg) => { setToast(msg); setTimeout(()=>setToast(null),2600); };

  const archived = useMemo(()=>tickets.filter(t=>t.archived),[tickets]);
  const cutoff = useMemo(()=>{ const d=new Date(); d.setDate(d.getDate()-PERIODS[period].days); return d; },[period]);
  const active = useMemo(()=>tickets.filter(t=>!t.archived&&parseDate(t.date)>=cutoff).sort((a,b)=>parseDate(b.date)-parseDate(a.date)),[tickets,cutoff]);
  const total = active.length;
  const stackH = total>0?CARD_H+PEEK*(total-1):100;
  const pStats = useMemo(()=>({count:active.length,total:active.reduce((s,t)=>s+t.amount,0)}),[active]);
  const sResults = useMemo(()=>searchQ.trim()?tickets.filter(t=>t.label.toLowerCase().includes(searchQ.toLowerCase())):[],[searchQ,tickets]);

  const sCutoff = useMemo(()=>{ const d=new Date(); d.setDate(d.getDate()-PERIODS[statPeriod].days); return d; },[statPeriod]);
  const sData = useMemo(()=>{
    const n=new Date(); const nm=statPeriod<=1?3:12;
    const filtered=tickets.filter(t=>parseDate(t.date)>=sCutoff);
    const bars=[];
    for(let i=nm-1;i>=0;i--){
      const m=new Date(n.getFullYear(),n.getMonth()-i,1);
      const mE=new Date(n.getFullYear(),n.getMonth()-i+1,0); mE.setHours(23,59,59,999);
      const v=tickets.filter(t=>{const d=parseDate(t.date);return d>=m&&d<=mE;}).reduce((s,t)=>s+t.amount,0);
      bars.push({l:MONTHS[m.getMonth()],v});
    }
  ...
  const submitManual = () => {
    ...
    setTickets(p=>[t,...p]); setForm({label:"",amount:"",date:"",items:""}); setAddMode(null); setAddSheet(false); toast_("Ticket ajoute !");
  };
  const del = (id) => { setTickets(p=>p.filter(t=>t.id!==id)); setDetailT(null); toast_("Supprime"); };
  const arch = (tid,fid) => { setTickets(p=>p.map(t=>t.id===tid?{...t,archived:true,archiveFolder:fid}:t)); setArchivePick(null); setDetailT(null); toast_("Archive"); };
  const unarch = (id) => { setTickets(p=>p.map(t=>t.id===id?{...t,archived:false,archiveFolder:undefined}:t)); setDetailT(null); toast_("Remis dans le wallet"); };
  const saveFolder = () => {
    if(!folderForm.label) return;
    if(editFolder){ setFolders(p=>p.map(f=>f.id===editFolder?{...f,...folderForm}:f)); }
    else { const cols=["#C9A84C","#6ee7b7","#f9a8d4","#93c5fd"]; setFolders(p=>[...p,{id:"f"+Date.now(),label:folderForm.label,sub:folderForm.sub||"",color:cols[p.length%cols.length]}]); }
    setFolderModal(false); setEditFolder(null); setFolderForm({label:"",sub:""}); toast_("Dossier sauvegarde");
  };
  const delFolder = (id) => { setFolders(p=>p.filter(f=>f.id!==id)); setTickets(p=>p.map(t=>t.archiveFolder===id?{...t,archived:false,archiveFolder:undefined}:t)); setArchiveOpen(null); toast_("Dossier supprime"); };

  return (
    <div style={{width:"100%",height:"100vh",background:theme.bg,overflow:"hidden",position:"relative",display:"flex",flexDirection:"column"}}>
      <style>{CSS}</style>
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative",width:"100%"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 26px 0",flexShrink:0}}>
          <span style={{color:"white",fontSize:15,fontWeight:700,fontFamily:"Outfit,sans-serif"}}>9:41</span>
          <div style={{width:100,height:28,background:"#000",borderRadius:18,border:"1px solid #333"}}/>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <svg width="16" height="11" viewBox="0 0 16 11" fill="white"><rect x="0" y="3" width="3" height="8" rx="1" opacity=".4"/><rect x="4.5" y="2" width="3" height="9" rx="1" opacity=".6"/><rect x="9" y="0" width="3" height="11" rx="1" opacity=".85"/><rect x="13.5" y="0" width="2.5" height="11" rx="1"/></svg>
            <svg width="22" height="11" viewBox="0 0 24 11" fill="none"><rect x="0.5" y="0.5" width="20" height="10" rx="3" stroke="white" strokeOpacity=".35"/><rect x="2" y="2" width="15" height="7" rx="1.5" fill="white"/></svg>
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden",animation:viewAnim==="in"?"viewIn 0.28s cubic-bezier(0.22,1,0.36,1) both":"viewOut 0.15s ease-in both",pointerEvents:viewAnim==="out"?"none":"auto"}}>
          ...
          {view==="home"&&<>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"20px 18px 200px",position:"relative"}}>
              {total===0
                ?<div style={{textAlign:"center",paddingTop:80,color:"rgba(255,255,255,0.15)"}}><Icon type="receipt" size={44} opacity={0.15}/><div style={{fontFamily:"Outfit,sans-serif",fontSize:13,marginTop:14,lineHeight:1.6}}>Aucun ticket<br/>Appuyez sur + pour en ajouter</div></div>
                :<div style={{position:"relative",height:stackH,margin:"0 auto",maxWidth:354}}>
                    {active.map((t,i)=><TicketCard key={t.id} ticket={t} index={i} total={total} onClick={()=>setDetailT(t)} uiTheme={uiTheme}/>)}
                  </div>
              }
            </div>
            <div style={{position:"absolute",bottom:56,left:0,right:0,zIndex:80,background:"rgba(10,10,10,0.95)",backdropFilter:"blur(24px)",borderTop:"1px solid rgba(255,255,255,0.07)",padding:"10px 18px 12px"}}>
              <div style={{display:"flex",gap:6,marginBottom:9}}>
                {PERIODS.map((p,i)=><button key={i} onClick={()=>setPeriod(i)} style={{flex:1,height:26,borderRadius:9,border:"none",cursor:"pointer",background:period===i?`${theme.accent}33`:"rgba(255,255,255,0.05)",color:period===i?theme.accent:"rgba(255,255,255,0.3)",fontSize:10,fontFamily:"Outfit,sans-serif",fontWeight:period===i?600:400}}>{p.label}</button>)}
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{color:"rgba(255,255,255,0.2)",fontSize:10,fontFamily:"Outfit,sans-serif"}}>Sur {PERIODS[period].label}</span>
                <div style={{display:"flex",gap:16,alignItems:"center"}}>
                  <div style={{textAlign:"center"}}><div style={{color:"white",fontSize:17,fontWeight:500,fontFamily:"Outfit,sans-serif",lineHeight:1}}>{fmt(pStats.total)}</div><div style={{color:"rgba(255,255,255,0.25)",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"Outfit,sans-serif",marginTop:2}}>depense</div></div>
                  <div style={{width:1,height:22,background:"rgba(255,255,255,0.08)"}}/>
                  <div style={{textAlign:"center"}}><div style={{color:"white",fontSize:17,fontWeight:500,fontFamily:"Outfit,sans-serif",lineHeight:1}}>{pStats.count}</div><div style={{color:"rgba(255,255,255,0.25)",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"Outfit,sans-serif",marginTop:2}}>tickets</div></div>
                </div>
              </div>
            </div>
          </>}
          ...
          {view==="search"&&<>
            <div style={{padding:"16px 18px 0",flexShrink:0}}>
              ...
              <button onClick={()=>{cv("home");setSearchQ("");}} style={{background:"none",border:"none",color:"rgba(255,255,255,0.4)",fontSize:12,fontFamily:"Outfit,sans-serif",cursor:"pointer",marginTop:8,padding:0}}>Retour</button>
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"10px 16px 80px"}}>
              ...
              {searchQ&&sResults.length===0&&<div style={{textAlign:"center",color:"rgba(255,255,255,0.18)",padding:"60px 0",fontSize:13,fontFamily:"Outfit,sans-serif"}}>Aucun resultat</div>}
              {sResults.map(t=>{
                const ct=getCT(uiTheme,t.cardTheme||0);
                return <div key={t.id} className="rrow" onClick={()=>setDetailT(t)} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:16,marginBottom:4,cursor:"pointer"}}>
                  <div style={{width:38,height:38,borderRadius:12,background:ct.grad,flexShrink:0}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{color:"white",fontSize:14,fontWeight:500,fontFamily:"Outfit,sans-serif"}}>{t.label}</div>
                    <div style={{color:"rgba(255,255,255,0.3)",fontSize:10,fontFamily:"Outfit,sans-serif",marginTop:2}}>{t.date}</div>
                  </div>
          ...
          {view==="archive"&&<>
            <div style={{padding:"14px 22px 10px",flexShrink:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <button onClick={()=>{setFolderForm({label:"",sub:""});setEditFolder(null);setFolderModal(true);}} style={{...HBtn,background:`linear-gradient(135deg,#F5DC70,${YF.main})`,width:36,height:36}}>
                  <Icon type="plus" size={16} color={YF.text}/>
                </button>
              </div>
              {archived.length>0&&<div style={{marginTop:10,padding:"8px 14px",background:"rgba(255,255,255,0.04)",borderRadius:12,border:"1px solid rgba(255,255,255,0.08)"}}><span style={{color:"rgba(255,255,255,0.4)",fontSize:11,fontFamily:"Outfit,sans-serif"}}>{archived.length} tickets archives - {fmt(archived.reduce((s,t)=>s+t.amount,0))}</span></div>}
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"0 16px 80px"}}>
              {folders.length===0&&<div style={{textAlign:"center",color:"rgba(255,255,255,0.2)",padding:"60px 0",fontSize:13,fontFamily:"Outfit,sans-serif"}}>Aucun dossier</div>}
              {folders.map((f,i)=>(
                <ArchiveFolder key={f.id} folder={f} index={i} tickets={archived.filter(t=>t.archiveFolder===f.id)} isOpen={archiveOpen===f.id} onToggle={()=>setArchiveOpen(archiveOpen===f.id?null:f.id)} onEdit={()=>{setFolderForm({label:f.label,sub:f.sub});setEditFolder(f.id);setFolderModal(true);}} onDelete={()=>delFolder(f.id)} uiTheme={uiTheme}/>
              ))}
            </div>
          </>}
          ...
          {view==="stats"&&<>
            ...
            <div style={{display:"flex",gap:6,padding:"12px 16px 0",flexShrink:0}}>
              {PERIODS.map((p,i)=><button key={i} onClick={()=>setStatPeriod(i)} style={{flex:1,height:28,borderRadius:9,border:"none",cursor:"pointer",background:statPeriod===i?`${theme.accent}33`:"rgba(255,255,255,0.05)",color:statPeriod===i?theme.accent:"rgba(255,255,255,0.3)",fontSize:10,fontFamily:"Outfit,sans-serif",fontWeight:statPeriod===i?600:400}}>{p.label}</button>)}
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"14px 16px 80px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                {[{label:"Total",value:fmt(sData.total),sub:PERIODS[statPeriod].label,color:theme.accent},{label:"Tickets",value:sData.count,sub:"transactions",color:"#10B981"},{label:"Moyenne",value:sData.count>0?fmt(sData.total/sData.count):"---",sub:"par ticket",color:"#F59E0B"},{label:"Max",value:sData.biggest?fmt(sData.biggest.amount):"---",sub:sData.biggest?.label||"---",color:"#EC4899"}].map((k,i)=>(
                  <div key={i} style={{background:theme.card,borderRadius:18,padding:"14px 16px",border:"1px solid rgba(255,255,255,0.08)"}}>
                    ...
                    <div style={{color:"white",fontSize:16,fontWeight:700,fontFamily:"Outfit,sans-serif"}}>{k.value}</div>
              ...
              <div style={{background:theme.card,borderRadius:18,padding:"16px",border:"1px solid rgba(255,255,255,0.08)"}}>
                <div style={{color:"rgba(255,255,255,0.3)",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"Outfit,sans-serif",marginBottom:12}}>Depenses</div>
                <BarChart data={sData.bars} color={theme.accent}/>
              </div>
            </div>
          </>}
        </div>
        ...
        {/* NAV */}
        <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(10,10,10,0.97)",backdropFilter:"blur(24px)",borderTop:"1px solid rgba(255,255,255,0.07)",zIndex:90}}>
          {activeTab==="theme"&&(
            <div style={{padding:"14px 14px 10px",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7}}>
                {Object.values(THEMES).map(t=>(
                  <button key={t.id} onClick={()=>{setUiTheme(t.id);setActiveTab("home");}} style={{padding:"9px 6px 8px",borderRadius:12,border:"none",cursor:"pointer",background:uiTheme===t.id?"rgba(255,255,255,0.12)":"rgba(255,255,255,0.04)",outline:uiTheme===t.id?"1.5px solid rgba(255,255,255,0.3)":"1.5px solid transparent"}}>
          ...
          <div style={{height:56,display:"flex",alignItems:"center"}}>
            {[{tab:"home",icon:"home",label:"Wallet"},{tab:"stats",icon:"stats",label:"Stats"},{tab:"archive",icon:"archive",label:"Archives"},{tab:"theme",icon:null,label:"Theme"}].map(({tab,icon,label})=>(
              <button key={tab} className="nbtn" onClick={()=>{ if(tab==="theme"){setActiveTab(p=>p==="theme"?"home":"theme");}else{setActiveTab(tab);cv(tab);} }} style={{flex:1,border:"none",background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,paddingBottom:4,position:"relative"}}>
                ...
                <span style={{color:activeTab===tab?"white":"rgba(255,255,255,0.3)",fontSize:9,fontFamily:"Outfit,sans-serif",fontWeight:activeTab===tab?600:400}}>{label}</span>
        ...
        {addSheet&&!addMode&&(
          <div style={{position:"absolute",inset:0,zIndex:180,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"flex-end",backdropFilter:"blur(8px)",animation:"fadeIn 0.2s"}} onClick={()=>setAddSheet(false)}>
            <div style={{width:"100%",background:"#111114",borderRadius:"28px 28px 0 0",padding:"0 18px 48px",animation:"slideUp 0.42s cubic-bezier(0.22,1,0.36,1)",border:"1px solid rgba(255,255,255,0.08)"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{color:"rgba(255,255,255,0.3)",fontSize:12,fontFamily:"Outfit,sans-serif",marginBottom:22}}>Comment souhaitez-vous ajouter ?</div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <button onClick={()=>setAddMode("manual")} style={{height:56,borderRadius:16,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.06)",color:"white",fontSize:14,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontFamily:"Outfit,sans-serif"}}>
                  <Icon type="plus" size={18}/> Saisie manuelle
                </button>
                <button onClick={()=>{toast_("Scan IA disponible sur Netlify");setAddSheet(false);}} style={{height:56,borderRadius:16,border:"none",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontFamily:"Outfit,sans-serif",boxShadow:"0 4px 18px rgba(99,102,241,0.35)"}}>
        ...
        {addSheet&&addMode==="manual"&&(
          <div style={{position:"absolute",inset:0,zIndex:180,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"flex-end",backdropFilter:"blur(8px)"}} onClick={()=>{setAddMode(null);setAddSheet(false);}}>
            <div style={{width:"100%",maxHeight:"82%",background:"#111114",borderRadius:"28px 28px 0 0",padding:"0 18px 44px",animation:"slideUp 0.42s cubic-bezier(0.22,1,0.36,1)",display:"flex",flexDirection:"column",overflow:"hidden",border:"1px solid rgba(255,255,255,0.08)"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{overflowY:"auto",flex:1}}>
                {[{k:"label",l:"Libelle *",ph:"Ex : Carrefour",m:"text"},{k:"amount",l:"Montant (EUR) *",ph:"Ex : 42.50",m:"decimal"},{k:"items",l:"Articles (separes par virgule)",ph:"Pain, Fromage...",m:"text"}].map(({k,l,ph,m})=>(
                  <div key={k} style={{marginBottom:12}}>
                    ...
                    <input value={form[k]} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} placeholder={ph} inputMode={m} style={{width:"100%",padding:"12px 14px",borderRadius:12,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.05)",color:"white",fontSize:14,fontFamily:"Outfit,sans-serif",outline:"none",boxSizing:"border-box"}}/>
                  </div>
                ))}
                <div style={{marginBottom:12}}>
                  <div style={{color:"rgba(255,255,255,0.35)",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6,fontFamily:"Outfit,sans-serif"}}>Date</div>
        ...
        {/* TICKET DETAIL ZIGZAG */}
        {detailT&&(()=>{
          const ct=getCT(uiTheme,detailT.cardTheme||0);
          const tc=ct.light?"rgba(10,5,0,0.9)":"rgba(255,255,255,0.95)";
          const sc=ct.light?"rgba(10,5,0,0.5)":"rgba(255,255,255,0.45)";
          return (
            <div style={{position:"absolute",inset:0,zIndex:160,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"flex-end",backdropFilter:"blur(8px)",animation:"fadeIn 0.2s"}} onClick={()=>setDetailT(null)}>
              <div style={{width:"100%",background:"#111114",borderRadius:"28px 28px 0 0",animation:"slideUp 0.42s cubic-bezier(0.22,1,0.36,1)",overflow:"hidden",border:"1px solid rgba(255,255,255,0.08)"}} onClick={e=>e.stopPropagation()}>
                {/* ZIGZAG HEADER */}
                <div className="rshape" style={{background:ct.grad,padding:"22px 22px 20px",border:`1px solid ${ct.light?"rgba(0,0,0,0.08)":"rgba(255,255,255,0.1)"}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                    <div>
                      <div style={{color:sc,fontSize:9,letterSpacing:2,textTransform:"uppercase",fontFamily:"'Space Mono',monospace",marginBottom:4}}>#{String(detailT.id).slice(-6).padStart(6,"0")}</div>
                      <div style={{color:tc,fontSize:22,fontWeight:700,fontFamily:"Outfit,sans-serif",letterSpacing:-0.5}}>{detailT.label}</div>
                      <div style={{color:sc,fontSize:11,fontFamily:"'Courier Prime',monospace",marginTop:3}}>{detailT.date}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{color:sc,fontSize:9,letterSpacing:2,textTransform:"uppercase",fontFamily:"'Space Mono',monospace",marginBottom:4}}>Total</div>
                  ...
                  <div style={{borderTop:`1px dashed ${sc}`,margin:"10px 0",opacity:0.4}}/>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <Barcode color={ct.light?"rgba(0,0,0,0.4)":"rgba(255,255,255,0.3)"} opacity={1}/>
                    <div style={{color:sc,fontSize:7,fontFamily:"'Space Mono',monospace",letterSpacing:0.5}}>TXN-{String(detailT.id).padStart(9,"0")}</div>
                  </div>
                </div>
                <div style={{padding:"16px 18px 0"}}>
                  {(detailT.items?.length||0)>0&&(
                  ...
                  <div style={{display:"flex",gap:8,paddingBottom:36}}>
                    {!detailT.archived
                      ?<button onClick={()=>{setArchivePick(detailT);setDetailT(null);}} style={{flex:1,height:48,borderRadius:14,border:"none",background:`linear-gradient(135deg,${YF.main},#E8C85A)`,color:"#0A0A00",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"Outfit,sans-serif"}}>
                          <Icon type="archive" size={15} color="#0A0A00"/> Archiver
                        </button>
                      :<button onClick={()=>unarch(detailT.id)} style={{flex:1,height:48,borderRadius:14,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"white",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"Outfit,sans-serif"}}>
                          <Icon type="inbox" size={15}/> Remettre dans le wallet
                        </button>
                    }
                    <button onClick={()=>del(detailT.id)} style={{width:48,height:48,borderRadius:14,border:"none",background:"rgba(255,59,48,0.1)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          ...
          <div style={{position:"absolute",inset:0,zIndex:200,background:"rgba(0,0,0,0.72)",display:"flex",alignItems:"flex-end",backdropFilter:"blur(8px)",animation:"fadeIn 0.2s"}} onClick={()=>setArchivePick(null)}>
            <div style={{width:"100%",background:"#111114",borderRadius:"28px 28px 0 0",padding:"0 18px 48px",animation:"slideUp 0.42s cubic-bezier(0.22,1,0.36,1)",border:"1px solid rgba(255,255,255,0.08)"}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {folders.map(f=>(
                  <button key={f.id} onClick={()=>arch(archivePick.id,f.id)} style={{height:52,borderRadius:14,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.05)",color:"white",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:12,padding:"0 16px",fontFamily:"Outfit,sans-serif"}}>
                    <div style={{width:10,height:10,borderRadius:3,background:f.color,flexShrink:0}}/>
                    <span style={{flex:1}}>{f.label}</span>
                    <span style={{color:"rgba(255,255,255,0.25)",fontSize:11}}>{archived.filter(t=>t.archiveFolder===f.id).length} tickets</span>
                  </button>
                ))}
...
const CSS = `
  @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;700&family=Space+Mono:wght@400;700&family=Courier+Prime:wght@400;700&display=swap');
  ...
  .rshape {
    clip-path: polygon(
      0% 2.5%,1% 0%,2% 2.5%,3% 0%,4% 2.5%,5% 0%,6% 2.5%,7% 0%,8% 2.5%,9% 0%,10% 2.5%,
      11% 0%,12% 2.5%,13% 0%,14% 2.5%,15% 0%,16% 2.5%,17% 0%,18% 2.5%,19% 0%,20% 2.5%,
      21% 0%,22% 2.5%,23% 0%,24% 2.5%,25% 0%,26% 2.5%,27% 0%,28% 2.5%,29% 0%,30% 2.5%,
      31% 0%,32% 2.5%,33% 0%,34% 2.5%,35% 0%,36% 2.5%,37% 0%,38% 2.5%,39% 0%,40% 2.5%,
      41% 0%,42% 2.5%,43% 0%,44% 2.5%,45% 0%,46% 2.5%,47% 0%,48% 2.5%,49% 0%,50% 2.5%,
      51% 0%,52% 2.5%,53% 0%,54% 2.5%,55% 0%,56% 2.5%,57% 0%,58% 2.5%,59% 0%,60% 2.5%,
      61% 0%,62% 2.5%,63% 0%,64% 2.5%,65% 0%,66% 2.5%,67% 0%,68% 2.5%,69% 0%,70% 2.5%,
      71% 0%,72% 2.5%,73% 0%,74% 2.5%,75% 0%,76% 2.5%,77% 0%,78% 2.5%,79% 0%,80% 2.5%,
      81% 0%,82% 2.5%,83% 0%,84% 2.5%,85% 0%,86% 2.5%,87% 0%,88% 2.5%,89% 0%,90% 2.5%,
      91% 0%,92% 2.5%,93% 0%,94% 2.5%,95% 0%,96% 2.5%,97% 0%,98% 2.5%,99% 0%,100% 2.5%,
      100% 97.5%,
      99% 100%,98% 97.5%,97% 100%,96% 97.5%,95% 100%,94% 97.5%,93% 100%,92% 97.5%,91% 100%,90% 97.5%,
      89% 100%,88% 97.5%,87% 100%,86% 97.5%,85% 100%,84% 97.5%,83% 100%,82% 97.5%,81% 100%,80% 97.5%,
      79% 100%,78% 97.5%,77% 100%,76% 97.5%,75% 100%,74% 97.5%,73% 100%,72% 97.5%,71% 100%,70% 97.5%,
      69% 100%,68% 97.5%,67% 100%,66% 97.5%,65% 100%,64% 97.5%,63% 100%,62% 97.5%,61% 100%,60% 97.5%,
      59% 100%,58% 97.5%,57% 100%,56% 97.5%,55% 100%,54% 97.5%,53% 100%,52% 97.5%,51% 100%,50% 97.5%,
      49% 100%,48% 97.5%,47% 100%,46% 97.5%,45% 100%,44% 97.5%,43% 100%,42% 97.5%,41% 100%,40% 97.5%,
      39% 100%,38% 97.5%,37% 100%,36% 97.5%,35% 100%,34% 97.5%,33% 100%,32% 97.5%,31% 100%,30% 97.5%,
      29% 100%,28% 97.5%,27% 100%,26% 97.5%,25% 100%,24% 97.5%,23% 100%,22% 97.5%,21% 100%,20% 97.5%,
      19% 100%,18% 97.5%,17% 100%,16% 97.5%,15% 100%,14% 97.5%,13% 100%,12% 97.5%,11% 100%,10% 97.5%,
      9% 100%,8% 97.5%,7% 100%,6% 97.5%,5% 100%,4% 97.5%,3% 100%,2% 97.5%,1% 100%,0% 97.5%
    );
  }
  .wcard:hover{transform:translateY(-6px)!important}
  .wcard:active{transform:scale(0.975)!important}
  .rrow:hover{background:rgba(255,255,255,0.06)!important}
